...
- Simple barrier redesigns were not enough, a naive percentile-to-binary target did not fix the collapse, and even top-versus-bottom ranking buckets still degenerated. The first direct-return regression prototype looked directionally more promising on the test top buckets, so the next useful step is now to formalize that path with a proper regression or multi-bucket ranking workflow.
