...
- [x] Save a recent operator output for the preferred NVDA watchlist overlay. Performance: `assets/nvda/operator_recent.tsv` now tracks the latest probabilities for the winning combo under the `top 12.5%` rule. The latest saved row on `2026-03-17` was not selected, with `predicted_probability=0.4926` versus the active cutoff `0.5148`.
- [x] Run a first regression prototype to check whether NVDA's stalled binary line hides a stronger ranking edge. Performance: `assets/nvda/regression_prototype_summary.tsv` showed that regression does produce a directional signal, but not one strong enough yet to replace the current binary candidate. The strongest test bucket was the `ret_60 + sma_gap_60 + atr_pct_20` regression on the `top` side, with `test_top10_avg_return=27.21%` versus overall `test_avg_return=16.53%`, but the correlation remained weak at `test_corr=0.0368`. So NVDA still has a better binary candidate than regression candidate for now.
...
- The first regression prototype was informative but not persuasive enough to replace the current binary combo, so NVDA should stay on the binary-watchlist path for now; if formulation work resumes, the `top 10%` regression candidate is the first place worth revisiting.
- Current live banding is not broken, but it is clearly conservative. On `2026-03-27`, NVDA only reached `predicted_probability=0.4310` versus `threshold=0.4020`, so the confidence gap was just `0.0290`. That sits well below the historical `strong_bullish` cutoff `0.0646` and `very_strong_bullish` cutoff `0.0735`, which is why the latest bar stayed `weak_bullish` instead of escalating.
- In other words, the missing `strong_bullish / very_strong_bullish` marks are currently a calibration outcome, not a display bug. The model is still producing mostly shallow above-threshold readings rather than high-conviction upside bursts, and that matches the broader NVDA conclusion that the line is usable as a watchlist candidate but not yet a robust promoted default.
- A full `1260`-bar chart audit clarified that the suppression is not only a threshold-band issue. NVDA raw model outputs did produce `80` `strong_bullish` bars and `31` `very_strong_bullish` bars, but the rendered chart only kept `1` `strong_bullish` bar and `0` `very_strong_bullish` bars after the buy-point overlay. In total, `694` bars were downgraded (`raw_model_signal != signal`), so the chart is currently showing the post-filtered execution view rather than the raw model-conviction view.
- If stronger color separation is important later, the right next step is not model retraining first; it is a chart / signal-policy choice. We should decide whether the HTML should keep showing only post-overlay execution signals, or whether it should offer a second raw-conviction layer so `strong_bullish` and `very_strong_bullish` remain visible even when the buy-point filter demotes them operationally.
