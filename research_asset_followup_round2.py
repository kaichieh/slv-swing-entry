...
def round2_validation_output_path(asset_key: str | None = None) -> Path:
    return ac.get_asset_dir(asset_key) / "followup_round2_validation_summary.tsv"


def round2_operator_output_path(asset_key: str | None = None) -> Path:
    return ac.get_asset_dir(asset_key) / "followup_round2_operator_summary.tsv"


def round2_decision_output_path(asset_key: str | None = None) -> Path:
    return ac.get_asset_dir(asset_key) / "followup_round2_decision_summary.tsv"
...
def load_round1_candidates() -> list[af.FollowupCandidate]:
    path = ac.get_asset_dir() / "candidate_validation_summary.tsv"
    if not path.exists():
        raise FileNotFoundError(f"Round-1 follow-up candidates not found at {path}")
...
def main() -> None:
    candidates = load_round1_candidates()
    specs = afr2.prioritize_round2_specs(afr2.build_round2_specs(candidates), limit=8)
    ...
    for spec in specs:
        ...
        walkforward = rb.evaluate_walk_forward_with_folds(
            ...
            spec.extra_features,
            folds=4,
            neg_weight=spec.neg_weight if spec.neg_weight is not None else pr.get_env_float("AR_NEG_WEIGHT", 1.0),
        )
        ...
        round2_score = afr2.compute_round2_score(
        ...
        validation_rows.append(
            {
                ...
                "round2_score": round_float(round2_score),
            }
        )
    ...
    validation_table = pd.DataFrame(validation_rows).sort_values("round2_score", ascending=False)
    ...
    validation_table.to_csv(round2_validation_output_path(), sep="\t", index=False)
    operator_table.to_csv(round2_operator_output_path(), sep="\t", index=False)
    decision_table.to_csv(round2_decision_output_path(), sep="\t", index=False)

    print(
        json.dumps(
            {
                "asset_key": ac.get_asset_key(),
                "round2_specs": len(validation_table),
                "top_model": validation_table.iloc[0]["model_name"] if len(validation_table) else "",
                "top_score": float(validation_table.iloc[0]["round2_score"]) if len(validation_table) else 0.0,
            },
            indent=2,
        )
    )
