...
import asset_followup_round3 as afr3
...
def round3_validation_output_path(asset_key: str | None = None) -> Path:
    return ac.get_asset_dir(asset_key) / "followup_round3_validation_summary.tsv"


def round3_operator_output_path(asset_key: str | None = None) -> Path:
    return ac.get_asset_dir(asset_key) / "followup_round3_operator_summary.tsv"


def round3_decision_output_path(asset_key: str | None = None) -> Path:
    return ac.get_asset_dir(asset_key) / "followup_round3_decision_summary.tsv"
...
def load_round2_candidates() -> list[af.FollowupCandidate]:
    path = ac.get_asset_dir() / "followup_round2_validation_summary.tsv"
    if not path.exists():
        raise FileNotFoundError(f"Round-2 follow-up candidates not found at {path}")
...
def main() -> None:
    candidates = load_round2_candidates()
    specs = afr3.prioritize_round3_specs(afr3.build_round3_specs(candidates), limit=10)
    ...
    for spec in specs:
        ...
        round3_score = afr3.compute_round3_score(
        ...
        validation_rows.append(
            {
                ...
                "round3_score": round_float(round3_score),
            }
        )
    ...
    validation_table = pd.DataFrame(validation_rows).sort_values("round3_score", ascending=False)
    ...
    validation_table.to_csv(round3_validation_output_path(), sep="\t", index=False)
    operator_table.to_csv(round3_operator_output_path(), sep="\t", index=False)
    decision_table.to_csv(round3_decision_output_path(), sep="\t", index=False)

    print(
        json.dumps(
            {
                "asset_key": ac.get_asset_key(),
                "round3_specs": len(validation_table),
                "top_model": validation_table.iloc[0]["model_name"] if len(validation_table) else "",
                "top_score": float(validation_table.iloc[0]["round3_score"]) if len(validation_table) else 0.0,
            },
            indent=2,
        )
    )
