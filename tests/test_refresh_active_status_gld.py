...
class RefreshActiveStatusGldTests(unittest.TestCase):
    ...
    def test_build_gld_accepts_configured_baseline_line_without_term_panic_overlay(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            cache_dir = Path(tmp)
            latest_prediction_path = cache_dir / "latest_prediction.json"
            latest_prediction_path.write_text(
                json.dumps(
                    {
                        ...
                        "live_provenance": {
                            "operator_line_id": "gld_current_live_mixed_live",
                            "left_expert": "dual_context",
                            "right_expert": "context_no_atr",
                            "outer_gate_feature": "atr_pct_20_percentile",
                            "outer_gate_threshold": 0.7,
        ...
        self.assertEqual(output.loc[0, "line_id"], "gld_current_live_mixed_live")
        self.assertIn("threshold_plus_buy_point_overlay", output.loc[0, "usage_note"])
        self.assertNotIn("vix_vxv_term_panic_block", output.loc[0, "usage_note"])

    def test_build_gld_rejects_baseline_line_when_stale_term_panic_overlay_fields_are_present(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            cache_dir = Path(tmp)
            latest_prediction_path = cache_dir / "latest_prediction.json"
            latest_prediction_path.write_text(
            ...
            with mock.patch.object(
                ...
                with mock.patch.object(ras.ac, "get_latest_prediction_path", return_value=latest_prediction_path):
                    with mock.patch.object(ras, "read_signal_rows_from_cache", return_value=signal_rows):
                        with self.assertRaisesRegex(ValueError, "GLD preferred line 'gld_current_live_mixed_live' cannot be validated"):
                            ras.build_gld(cache_dir)
